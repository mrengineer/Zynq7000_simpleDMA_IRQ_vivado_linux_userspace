...
always @(posedge sys_clk) begin
    if (!sys_rst_n) begin
     ...
     //   if (m_axis_tready) begin        //IF AXI DMA IS READY

            r_m_axis_tvalid <= 1'b1;
            
            r_m_axis_tlast  <= (r_s_axis_tdata == 7) ? 1'b1 : 1'b0;
            r_s_axis_tdata  <= (r_s_axis_tdata == 8) ? 4'b0001 : r_s_axis_tdata + 1;       //per each 8 values made of 32 bit
        

        if (m_axis_tready) begin
            r_last <= (r_cnt == 7) ? 1 : 0;
            r_cnt  <= (r_cnt == 8) ? 4'b0001 : r_cnt + 1;       //per each 8 values made of 32 bit

        end else begin
            r_last <= 0;
            r_cnt  <= (r_cnt >= 8) ? 4'b0001 : r_cnt;
        end
       // end else begin
         //   r_m_axis_tvalid <= 1'b0;
           // r_m_axis_tlast  <= 1'b0;
